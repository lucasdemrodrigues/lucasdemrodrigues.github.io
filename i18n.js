(() => {
  ...
  const q=(sel,root=document)=>root.querySelector(sel),qa=(sel,root=document)=>[...root.querySelectorAll(sel)],text=(sel,v,root=document)=>{const e=q(sel,root);if(e&&v!=null)e.textContent=v},lead=(el,v)=>{if(!el)return;const n=[...el.childNodes].find(x=>x.nodeType===Node.TEXT_NODE);if(n)n.nodeValue=`${v} `};
  const focusTitle=(el,v)=>{if(!el)return;const parts=v.split(' & ');el.textContent='';el.append(parts[0]);if(parts.length>1){const amp=document.createElement('span');amp.className='plain-amp';amp.textContent=' & ';el.append(amp,parts.slice(1).join(' & '))}};
  const projectRepo=card=>card.dataset.repo||((q('.text-link',card)?.href||'').match(/github\.com\/[^/]+\/([^/?#]+)/)?.[1]||'');
  const localProjectText=(m,l)=>{if(!m)return null;const suffix=l==='pt'?'':`_${l}`;return{title:(m[`title${suffix}`]||m.title||'').trim(),description:(m[`description${suffix}`]||m.description||'').trim(),eyebrow:(m[`eyebrow${suffix}`]||m.eyebrow||'').trim()}};
  const projectMetadata=()=>new Map((window.portfolioProjects?.projects||[]).map(item=>[item.repo.name,item.metadata]));

  const translateProjects=()=>{
    const c=t[lang],filters=q('.project-filters');
  ...
  const syncTheme=()=>{const b=q('.theme-toggle');if(!b)return;const light=document.body.classList.contains('light-mode'),c=t[lang];b.setAttribute('aria-label',light?c.theme[0]:c.theme[1]);b.title=light?c.theme[2]:c.theme[3]};
  const syncCopyLabel=()=>{const box=q('.copy-email'),label=q('.copy-label'),status=q('.copy-status');if(!box||!label)return;const copied=box.classList.contains('copied');label.textContent=copied?t[lang].contact[4]:t[lang].contact[3];if(status)status.textContent=copied?t[lang].copyAnnounce:''};
  const syncMenuLabel=()=>{const b=q('.menu-toggle');if(!b)return;b.setAttribute('aria-label',b.getAttribute('aria-expanded')==='true'?t[lang].menu[1]:t[lang].menu[0])};
  ...
  const apply=l=>{
    ...
    text('#projetos .section-kicker',c.projects);translateProjects();text('#certificacoes .section-kicker',c.certifications[0]);qa('#certificacoes .featured-cert-link').forEach(e=>lead(e,c.certifications[1]));lead(q('#certificacoes .featured-certs-all'),c.certifications[2]);
    ...
    syncCopyLabel();const fs=qa('footer > span');if(fs[1])fs[1].textContent=c.footer;syncTheme();syncMenuLabel();qa('.language-switch button').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.lang===lang)));
