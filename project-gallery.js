(() => {
  ...
  const normalizeMetadata = (repo, metadata) => ({
    ...
    eyebrow: metadata.eyebrow || metadata.categories?.[0]?.toUpperCase() || 'PROJETO'
  });
  ...
  const createCard = (repo, metadata) => {
    const data = normalizeMetadata(repo, metadata);
    ...
    if (data.image) {
      ...
      img.src = data.image;
      img.alt = `Prévia do projeto ${data.title}`;
      img.loading = 'lazy';
      visual.appendChild(img);
      article.appendChild(visual);
    } else {
      article.appendChild(makeFallbackVisual(data.title));
    }

    const info = document.createElement('div');
    info.className = 'project-info';
    info.innerHTML = `
      <div class="project-meta"><span>${escapeHTML(data.eyebrow)}</span><span>GITHUB</span></div>
      <h3>${escapeHTML(data.title)}</h3>
      <p>${escapeHTML(data.description)}</p>
      <div class="project-tags">${data.tags.map(tag => `<span>${escapeHTML(tag)}</span>`).join('')}</div>
      <a class="text-link" href="${escapeHTML(repo.html_url)}" target="_blank" rel="noreferrer" aria-label="Ver ${escapeHTML(data.title)} no GitHub, abre em nova aba">Ver projeto no GitHub <b>↗</b></a>`;
  ...
  const fetchPortfolioMetadata = async repo => {
    ...
    try {
      ...
      return metadata.portfolio === true ? metadata : null;
    } catch (error) {
      console.warn(`portfolio.json inválido ou indisponível em ${repo.name}`, error);
      return null;
    }
  };
  ...
  style.textContent = `
    #projetos .section-head{margin-bottom:0}.projects-status.projects-status-compact{justify-content:flex-start;margin:25px 0 25px}
    ...
    @media(max-width:760px){.projects-grid{grid-template-columns:1fr}.project-filters{width:100%;overflow-x:auto;flex-wrap:nowrap;scrollbar-width:none}.project-filters::-webkit-scrollbar{display:none}.project-filter{white-space:nowrap}.projects-grid .project-info h3{font-size:22px!important}}@media(prefers-reduced-motion:reduce){.project-filter,.projects-grid .project-feature{transition:none!important}.projects-grid .project-feature.is-filtering-in{animation:none!important}}
  `;
